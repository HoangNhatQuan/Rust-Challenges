...
impl Solution {
    fn strobogrammatic_numbers(low: i32, high: i32) -> i32 {
        ...
        for len in low_str.len()..=high_str.len() {
            let mut c = vec![' '; len];
            Solution::recursion(&low_str, &high_str, &mut c, 0, len - 1, &mut count);
        }
    ...
    fn recursion(
        ...
        if left > right {
            ...
            if (s.len() == low.len() && s < low.to_string())
                || (s.len() == high.len() && s > high.to_string())
            {
                return;
            }
            *count += 1;
        ...
        for p in &PAIRS {
            ...
            Solution::recursion(low, high, c, left + 1, right - 1, count);
        }
    }
}
